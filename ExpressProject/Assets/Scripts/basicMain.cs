...
using UnityEngine.Networking;   // 유니티 Nextworking 사용
...
public class basicMain : MonoBehaviour
{
    ...
    void Start()
    {
        this.Hello.onClick.AddListener(()=>
        {
            ...
            Debug.Log(url);
            StartCoroutine(this.GetBasic(url, (raw) =>              // 코루틴으로 Getasic 호출 후에 raw 파일을 콜백에 넣는다.
            {
    ...
    private IEnumerator GetBasic(string url, System.Action<string> callback)
    {
        ...
        yield return webRequest.SendWebRequest();                   // 요청한 것이 돌아올 때 까지 대기
